...
public class PerlinNoiseMap : MonoBehaviour
{
    ...
    void UpdateForCamera(){
        xScreenLeft = cam.ScreenToWorldPoint(new Vector2(0, 0)).x;
        xScreenRight = cam.ScreenToWorldPoint(new Vector2(Screen.width, 0)).x;
        yScreenTop = cam.ScreenToWorldPoint(new Vector2(0, Screen.height)).y;
        yScreenBottom = cam.ScreenToWorldPoint(new Vector2(0, 0)).y;
        player = GameObject.FindWithTag("Player");
        //Debug.Log("diff: " + (xScreenLeft - transform.position.x ));
        if (!clonedToTheRight)
        if (xScreenLeft - transform.position.x > 5){
            Debug.Log("YEP");
            posToCopy = new Vector2(gameObject.transform.position.x + 30,gameObject.transform.position.y);
            GameObject instance = Instantiate(gameObject, posToCopy, Quaternion.identity);
            instance.name = xScreenLeft.ToString();
            clonedToTheRight = true;
            terrainsParent = GameObject.FindWithTag("Terrains");
            instance.transform.parent = terrainsParent.transform;
        }
    }
